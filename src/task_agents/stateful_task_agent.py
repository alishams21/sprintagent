"""Stateful task planning agent using planner/designer/critic workflow.

This agent mirrors the multi-agent pattern used in project/module agents, but
operates on task-level plans (single task with optional subtasks):

- Designer: creates and refines the task definition, subtasks, and acceptance criteria
- Critic: evaluates the task plan and suggests improvements with scored feedback
...
from src.agent_utils.scoring import TaskCritiqueWithScores
from src.task_agents.base_task_agent import BaseTaskAgent
from src.prompts import TaskAgentPrompts, prompt_registry

console_logger = logging.getLogger(__name__)


class StatefulTaskAgent(BaseStatefulAgent, BaseTaskAgent):
    """Stateful task planning agent using planner/designer/critic workflow."""
    ...
    def _get_initial_design_prompt_enum(self) -> Any:
        ...
        return TaskAgentPrompts.DESIGNER_INITIAL_INSTRUCTION

    def _get_initial_design_prompt_kwargs(self) -> dict:
        """Template variables for initial design prompt (none for task agent)."""
        return {}
    ...
    def _create_designer_agent(self, tools: list[FunctionTool] | None = None) -> Agent:
        """Create the task designer agent."""
        if tools is None:
            tools = []

        return super()._create_designer_agent(
            tools=tools,
            prompt_enum=TaskAgentPrompts.DESIGNER_AGENT,
            task_prompt=self.task_prompt,
        )

    def _create_critic_agent(self, tools: list[FunctionTool] | None = None) -> Agent:
        """Create the task critic agent."""
        if tools is None:
            tools = []

        return super()._create_critic_agent(
            tools=tools,
            prompt_enum=TaskAgentPrompts.CRITIC_AGENT,
            output_type=TaskCritiqueWithScores,
            task_prompt=self.task_prompt,
        )
    ...
    def _create_planner_agent(self, tools: list[FunctionTool]) -> Agent:
        ...
        return super()._create_planner_agent(
            tools=tools,
            prompt_enum=TaskAgentPrompts.PLANNER_AGENT,
            task_prompt=self.task_prompt,
    ...
    async def generate_task_plan(self, prompt: str, output_dir: Path) -> str:
        """Generate a task plan for the given prompt.

        This method:
        1. Initializes designer, critic, and planner agents
        2. Runs the planner with its runner instruction
        3. Saves the final plan to ``output_dir/task_plan.md`` and returns it
        """
        console_logger.info("Starting task planning workflow")
        ...
        runner_instruction = prompt_registry.get_prompt(
            prompt_enum=TaskAgentPrompts.PLANNER_RUNNER_INSTRUCTION,
        )
